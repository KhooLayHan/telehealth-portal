...
public class DoctorConfiguration : IEntityTypeConfiguration<Doctor>
{
    ...
    {
        var consultationFee = builder.Metadata.FindProperty(nameof(Doctor.ConsultationFee))!.GetColumnName();
        var deletedAtColumn = builder.Metadata.FindProperty(nameof(Doctor.DeletedAt))!.GetColumnName();

        builder.ToTable(
            "doctors",
            t =>
                t.HasCheckConstraint("chk_doctors_fee_nonnegative",
        ...
        builder.HasIndex(d => d.UserId).IsUnique().HasFilter($"{deletedAtColumn} is null")
        ...
        builder.HasIndex(d => d.LicenseNumber).IsUnique().HasFilter($"{deletedAtColumn} is null")
        ...
        builder.ComplexProperty(d => d.Qualifications, c =>
        { 
            c.ToJson();
        });
