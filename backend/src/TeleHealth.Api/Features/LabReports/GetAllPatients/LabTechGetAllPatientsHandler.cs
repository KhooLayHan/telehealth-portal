...
public sealed class LabTechGetAllPatientsHandler(ApplicationDbContext db)
{
    private const int MaxPageSize = 50;

    public async Task<PagedResult<LabTechPatientDto>> HandleAsync(
        LabTechGetAllPatientsQuery query,
    ...
    )
    {
        ...
        if (skip >= totalCount)
        {
            return new PagedResult<LabTechPatientDto>([], totalCount, page, pageSize);
        }

        var entities = await q.Skip((int)skip).Take(pageSize).ToListAsync(ct);
        var items = entities.Select(LabTechPatientDto.FromEntity).ToList();

        return new PagedResult<LabTechPatientDto>(items, totalCount, page, pageSize);
    }
}
