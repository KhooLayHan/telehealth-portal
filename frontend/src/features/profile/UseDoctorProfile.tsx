...
function validateForm(data: ProfileFormData): ProfileFormErrors {
  ...
  if (data.address && ADDRESS_BANNED_RE.test(data.address)) {
    errors.address = "Address may not contain special characters like % $ # @ !";
  }

  return errors;
}

async function fetchDoctorProfile(): Promise<MeData> {
  const res = await fetch(`${API_BASE}/api/v1/users/me/doctor`, { credentials: "include" });
