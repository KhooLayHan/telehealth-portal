...
import { Camera, Loader2, Mail, Pencil, Stethoscope, User, X } from "lucide-react";
import type * as React from "react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { type ProfileFormData, UseDoctorProfile } from "./UseDoctorProfile";
...
export function DoctorProfilePage() {
  ...
  return (
    ...
    >
            ...
            <CardContent className="flex flex-col gap-5 px-6 pb-6 pt-7">
              ...
              {isEditing ? (
                ...
                <>
                  ...
                  <div className="h-px bg-border" />
                  <InfoRow label="Address" value={me.address} />
                </>
              )}
            </CardContent>
          </Card>
      ...
      {/* Doctor credentials (read-only) */}
      {(me.licenseNumber || me.specialization) && (
        <motion.div variants={cardAnim}>
          <Card className="relative overflow-hidden">
            <div className="absolute inset-x-0 top-0 h-0.75" style={{ background: ACCENT }} />
            <CardContent className="px-6 pb-6 pt-7">
              <div className="mb-3 flex items-center gap-2">
                ...
                <SectionLabel>Doctor Credentials</SectionLabel>
              </div>
              <div className="grid grid-cols-2 gap-5">
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] text-muted-foreground/60 uppercase tracking-[0.15em]">
                  ...
                  <span className="font-medium text-sm">{me.licenseNumber || "—"}</span>
                  <p className="text-[10px] text-muted-foreground/50 mt-0.5">Read-only</p>
                </div>
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] text-muted-foreground/60 uppercase tracking-[0.15em]">
                    Specialization
                  </span>
                  <span className="font-medium text-sm capitalize">{me.specialization || "—"}</span>
                  <p className="text-[10px] text-muted-foreground/50 mt-0.5">Read-only</p>
                </div>
              </div>
            </CardContent>
